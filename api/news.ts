...
async function fetchGoogleNews(keyword: string = '쿠팡'): Promise<NewsItem[]> {
  try {
...
async function fetchNaverNews(keyword: string = '쿠팡'): Promise<NewsItem[]> {
  try {
    const encodedKeyword = encodeURIComponent(keyword);
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    return res.status(200).json({
      success: true,
      data: sortedNews,
      message: `${sortedNews.length}개의 뉴스를 가져왔습니다.`
    });
  } catch (error) {
